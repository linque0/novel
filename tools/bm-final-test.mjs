...
await sleep(400)
if (!(await waitFor(`document.querySelectorAll('.book-card').length`, 15000))) {
  console.error('书架未加载')
  process.exit(1)
}

/* 1. 侧边栏右键章节 → 快捷栏含书签项（先在正文页签定位到章节树） */
await cdp.eval(`document.querySelectorAll('.rail-item')[0].click()`)
await sleep(500)
if (!(await waitFor(`document.querySelectorAll('.n-tree-node').length`, 8000))) {
  console.error('章节树未加载')
  process.exit(1)
}
await cdp.eval(`(() => {
...
v = await cdp.eval(`({
  ...
  toolbar: [...document.querySelectorAll('.ob-toolbar .tb')].some(b => b.textContent.trim() === '★' && b.className.includes('on'))
})()`)
check('树 ★ 前缀与工具栏书签态', v.tree && v.toolbar, JSON.stringify(v))

/* 4. 工具栏书签键再点 → 移除 → 再收藏（恢复） */
await cdp.eval(`(() => {
  const btn = [...document.querySelectorAll('.ob-toolbar .tb')].find(b => b.title === '移除书签')
  btn?.click()
})()`)
await sleep(300)
v = await cdp.eval(`(async () => {
...
await cdp.eval(`(() => {
  const btn = [...document.querySelectorAll('.ob-toolbar .tb')].find(b => b.title === '收藏本书签')
  btn?.click()
})()`)
await sleep(300)
