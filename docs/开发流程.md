...
| 核验整合结果 | `node tools/shelf-verify.mjs`（15/15 在架 + 无未注册残留） |
| 验收脚本取种子 | `c.evalx(buildSeedExpr('书名', { reset: true }))`——与书架同源，改一处两侧同步 |

**验证书清单（15 本，v1.0.20 收编）**：双链验证书（dl-verify）· 批注验证书（annotation-verify）· 关系图验证书（chargraph-verify）· 画布验证书A/B（olcanvas-verify）· 关系跟随验证书（relfollow-verify）· 多窗口验证书（multiwindow-verify）· 画布右键验证书（canvasctx-verify）· 连接点修复验证书（junction-fix / freeze-repro / visual-fix 三脚本共用）· 线连到线验证书（line2line-verify）· 塑形验证书（shape-verify）· 浮层避线验证书（edgepanel-avoid-verify）· 右键浮层验证书（edgeedit-ctx-verify）· 侧栏验证书（sidebar-verify）· 验收测试作品（cdp-test）。

**新增验证书的纪律**：一律先在 `VERIFY_BOOKS` 登记（title / owner / create），再在 `docs/开发流程.md` 清单补一行；`create` 必须自包含（只允许用 `ctx = {db, uid, now}` 与 `meta = {title}`，不得引用模块外变量——注册表会被 `String()` 序列化到页面内求值）、只做纯创建（重置逻辑统一在 `seedBooks`）。**禁止在验收脚本里内联 `db.works.add` 另起炉灶**；演示书《星尘旅人》（产品截图用）走 `tools/seed-demo.mjs`，不属验证书体系。
...
| `bugfix-verify.mjs` | 14 | v1.0.22 缺陷修复专项（恢复时序 / 视图切换 / 幕布合并与按钮 / 写回门禁 / stripTags 等） |
| `annotation-verify.mjs` | 28 | 正文批注（右键添加 / 彩色下划线 / 侧栏 / 与双链共存 / 落库持久化） |
| `shelf-verify.mjs` | 15 | 验证书整合（15 本在架 + 无未注册残留，v1.0.20） |
